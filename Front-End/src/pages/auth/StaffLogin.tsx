...
// Schema validation cho form đăng nhập staff
...
export default function StaffLogin() {
  ...
  const staffLoginMutation = useMutation<AuthResponse>(authService.adminLogin, {
  ...
  const onSubmit = async (data: StaffLoginFormData) => {
    ...
    await staffLoginMutation.mutate(loginRequest);
  };

  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-blue-100 flex">
      <div className="hidden lg:flex lg:w-3/5 bg-linear-to-br from-blue-800 to-blue-900 p-12 flex-col justify-center">
        <div className="mb-8">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 p-8 shadow-lg">
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              <ShoppingCart className="w-6 h-6 text-blue-300 mt-1 shrink-0" />
              <p className="text-white">
                Quản lý và xử lý{" "}
                <span className="font-bold text-blue-300">đơn hàng</span> một
                cách nhanh chóng
              </p>
            </div>
            <div className="flex items-start space-x-3">
              <Package className="w-6 h-6 text-blue-300 mt-1 shrink-0" />
              <p className="text-white">
                Cập nhật thông tin{" "}
                <span className="font-bold text-blue-300">sản phẩm</span> và kho
                hàng
              </p>
            </div>
            <div className="flex items-start space-x-3">
              <Users className="w-6 h-6 text-blue-300 mt-1 shrink-0" />
              <p className="text-white">
                Hỗ trợ và tư vấn{" "}
                <span className="font-bold text-blue-300">khách hàng</span> tận
                tâm
              </p>
            </div>
            <div className="flex items-start space-x-3">
              <Shield className="w-6 h-6 text-blue-300 mt-1 shrink-0" />
              <p className="text-white">
          ...
          <div className="mt-4">
            <a
              href="#"
              className="text-blue-300 font-medium hover:text-blue-200"
            >
              Xem hướng dẫn làm việc &gt;
            </a>
          </div>
        </div>
      ...
      {/* Right Section - Staff Login Form */}
      <div className="w-full lg:w-2/5 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">Dành cho nhân viên cửa hàng</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Button
              ...
              disabled={staffLoginMutation.isLoading}
            >
              {staffLoginMutation.isLoading ? (
