...
const ArticleHomePage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const featuredResponse: ArticleListResponse =
        await articleService.getArticles(1, 4, "", true, null, null);
      setFeaturedArticles(featuredResponse.data?.data ?? []);

      const latestResponse: ArticleListResponse =
        await articleService.getArticles(1, 20, "", true, null, null);
