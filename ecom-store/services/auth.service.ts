...
export const authService = {
  ...
  socialLoginCallback: async (login_type: string, code: string, redirect_uri?: string) => {
    const params: any = { login_type, code };
    if (redirect_uri) {
      params.redirect_uri = redirect_uri;
    }
    const response = await axiosClient.get<AuthResponse>(`/auth/social-login/callback`, { params });
    return response;
  },
  
  socialLogin: async (login_type: string, redirect_uri?: string) => { 
    const params: any = { login_type };
    if (redirect_uri) {
      params.redirect_uri = redirect_uri;
    }
    const response = await axiosClient.get<ResponseApi<string>>(`/auth/social-login`, { params });
    return response;
  },
