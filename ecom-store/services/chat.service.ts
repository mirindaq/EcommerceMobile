...
export const chatService = {
  createChat: async () => {
    const response = await axiosClient.post<ChatResponse>("/chats");
  ...
  getChatByCustomerId: async (customerId: number) => {
    const response = await axiosClient.get<ChatResponse>(
  ...
  getMyChat: async () => {
    const response = await axiosClient.get<ChatResponse>("/chats/my-chat");
    return response.data;
  },

  getChatsByStaffId: async (staffId: number) => {
    const response = await axiosClient.get<ChatListResponse>(
      `/chats/staff/${staffId}`
    );
    return response.data;
  },
  ...
  getMessagesByChatId: async (chatId: number) => {
    const response = await axiosClient.get<MessageListResponse>(
  ...
  getUnreadMessageCount: async (chatId: number, userId: number) => {
    const response = await axiosClient.get<UnreadCountResponse>(
      `/chats/${chatId}/unread-count/${userId}`
    );
    return response.data;
  },
};
