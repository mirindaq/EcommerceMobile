...
import { Bot, ArrowLeft, Sparkles } from "lucide-react-native";
...
export default function AIChatScreen() {
  ...
  const handleSendMessage = async (message: string, messageType?: "TEXT" | "IMAGE") => {
    ...
    >
      <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
        <Box className="flex-1 bg-white">
          <Box className="bg-purple-600 px-4 py-4">
            ...
            {messages.length === 0 ? (
              <Box className="items-center py-8">
                ...
                <VStack className="items-start gap-1 mt-2">
                  <Text className="text-sm text-gray-600">• Tư vấn sản phẩm phù hợp</Text>
                  <Text className="text-sm text-gray-600">• Trạng thái đơn hàng</Text>
                  <Text className="text-sm text-gray-600">• Hướng dẫn sử dụng, bảo hành</Text>
                </VStack>
              </Box>
            ) : (
